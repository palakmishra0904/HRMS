<?php
	...
	if(isset($_POST['clientlogin']))
	{
		...
		{
			header("location:../user/index.php?msg=Username or Password is required");exit;
		}
		else
		{
	...
	else if(isset($_POST['submit']))
	{
		...
		else
		{
			...
			{
				header("location:../index.php?msg=Username or Password is Wrong!");exit;
			}
		}
	}

	else if(isset($_POST['HRsubmit']))
	{
		$Username=$_POST['name'];
		$Password=$_POST['password'];
		...
		if(empty($Username) || empty($Password))
		{
			...
			header("location:../HRindex.php?msg=UserName and Password is required");exit;
		}
		else
		{
			$sql = mysqli_query($db,"select * from employee where Email='$Username' AND Password='$Password' AND RoleId=2 ");
			...
			if(mysqli_num_rows($sql) > 0)
			{
				...
				$ro = mysqli_fetch_assoc($sqll);
				$_SESSION['role'] = $ro;
				header('location:../HRhome.php');exit;
			}
			else
			{
				header("location:../HRindex.php?msg=Username or Password is Wrong!");exit;
			}
		}
	}
	...
	function user()
	{
		...
		{
			header("location:../user/index.php?msg=Username or Password is Wrong!");exit;
		}
	}
?>
